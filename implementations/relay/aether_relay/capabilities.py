"""Capability token helpers for relay enforcement."""
...
def enforce_capability(
    ...
    *,
    ...
    chain = list(tokens)
    verify_chain(chain, now_ns=now_ns, usage=usage)
    if chain[-1].capability != required:
        raise ValueError("capability not granted")
